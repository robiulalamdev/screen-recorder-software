...
function Toggle({
  ...
  return (
    <button
      ...
      className="w-10 h-[22px] rounded-full p-0.5 transition-colors shrink-0"
      style={{
        backgroundColor: enabled ? "var(--accent)" : "var(--bg-elevated)",
      }}
    >
      <div
        className={`w-[18px] h-[18px] rounded-full bg-white shadow transition-transform ${enabled ? "translate-x-[18px]" : "translate-x-0"}`}
      />
    </button>
  );
}
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-center gap-2">
          <div className={inputCls + " truncate"} style={inputStyle}>
            {settings.saveLocation}
          </div>
          <button
            onClick={() =>
              invoke<string>("select_folder")
                .then((f) => f && updateSettings({ saveLocation: f }))
                .catch(console.error)
            }
      ...
      <div className="space-y-4">
        {[
          {
            label: "Auto create folders by Year/Month",
            key: "autoCreateFolders" as const,
          },
          {
            label: "Auto open folder after recording",
            key: "autoOpenFolder" as const,
          },
        ...
        <div className="flex gap-2">
          {(["dark", "light", "system"] as const).map((t) => (
            <button
              ...
              style={btnStyle(settings.theme === t)}
            >
              {t === "dark" && (
                <span
  ...
  return (
    <div className="space-y-6">
      {[
        ...
        {
          label: "Resolution",
          options: ["original", "1080p", "1440p", "4k"] as const,
          key: "resolution" as const,
          uppercase: true,
        },
        {
          label: "Encoder",
          options: ["h264", "h265", "av1"] as const,
          key: "encoder" as const,
          uppercase: true,
        },
        {
          label: "Output Format",
          options: ["mp4", "webm", "mkv"] as const,
        ...
        <div key={key}>
          ...
          <div className="flex flex-wrap gap-2">
            {options.map((opt) => (
              <button
                key={opt}
                onClick={() => updateSettings({ [key]: opt })}
                className={optionBtnCls + (uppercase ? " uppercase" : "")}
                style={btnStyle(settings[key] === opt)}
              >
                {opt}
                {suffix}
              </button>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) => updateSettings({ systemAudio: e.target.value })}
          className="w-full px-3 py-2 rounded-lg border text-sm outline-none"
          style={inputStyle}
        >
          <option>Default</option>
        ...
        {shortcutItems.map((item) => (
          ...
          >
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <button
                onClick={() => setListeningFor(item.key)}
...
export default function Settings({ activeTab }: SettingsProps) {
  ...
  return (
    <div className="flex flex-col sm:flex-row h-full">
      <div
        className="flex sm:flex-col overflow-x-auto sm:overflow-x-visible p-2 sm:p-4 shrink-0"
        style={{ borderBottom: "1px solid var(--border-primary)" }}
      >
        <div className="flex sm:flex-col gap-1 w-full">
          {tabs.map((tab) => (
            <button
              ...
              style={{
                backgroundColor:
                  currentTab === tab.id ? "var(--accent-bg)" : "transparent",
                color:
                  currentTab === tab.id
                    ? "var(--accent-text)"
                    : "var(--text-secondary)",
